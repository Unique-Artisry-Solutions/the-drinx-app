...
describe('update-subscription edge function security', () => {
  ...
  it('Rate limit: allows first two, blocks third within window', () => {
    const id = `update-subscription-${Date.now()}`;
  ...
  it('Auth: source includes explicit missing auth handling', () => {
    const src = fs.readFileSync('supabase/functions/update-subscription/index.ts', 'utf8');
