...
describe('notifications edge function security', () => {
  ...
  it('Rate limit: allows first two, blocks third within window', () => {
    const id = `notifications-${Date.now()}`;
  ...
  it('Auth: source includes explicit missing/unauthorized handling', () => {
    const src = fs.readFileSync('supabase/functions/notifications/index.ts', 'utf8');
