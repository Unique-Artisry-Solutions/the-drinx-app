...
const MobileProfilePage: React.FC = () => {
  ...
  const checkForActiveSwigCircuit = () => {
    ...
    if (!hasActive && Math.random() > 0.5) {
      // 50% chance to show an active circuit
  ...
  return <Layout>
      <div className="animate-fade-in relative overflow-hidden">
        ...
        {hasActiveSwigCircuit && <div className="mb-4 mt-2">
        ...
        <div className="bg-gradient-to-r from-purple-50 to-pink-50 dark:from-gray-800 dark:to-gray-900 p-3 rounded-xl mb-6 shadow-sm backdrop-blur-sm bg-white/30">
          <Tabs defaultValue="overview" className="space-y-3">
            <TabsList className="w-full flex justify-between bg-white/70 dark:bg-gray-800/50 backdrop-blur-sm p-1 rounded-lg">
              <TabsTrigger className="flex-1 text-[11px] data-[state=active]:bg-white dark:data-[state=active]:bg-gray-700 data-[state=active]:shadow-sm transition-all hover:bg-white/90 dark:hover:bg-gray-700/80" value="overview">
                Overview
              </TabsTrigger>
              <TabsTrigger className="flex-1 text-[11px] data-[state=active]:bg-white dark:data-[state=active]:bg-gray-700 data-[state=active]:shadow-sm transition-all hover:bg-white/90 dark:hover:bg-gray-700/80" value="activity">
                Activity
              </TabsTrigger>
              <TabsTrigger className="flex-1 text-[11px] data-[state=active]:bg-white dark:data-[state=active]:bg-gray-700 data-[state=active]:shadow-sm transition-all hover:bg-white/90 dark:hover:bg-gray-700/80" value="rewards">
                Rewards
              </TabsTrigger>
              <TabsTrigger className="flex-1 text-[11px] data-[state=active]:bg-white dark:data-[state=active]:bg-gray-700 data-[state=active]:shadow-sm transition-all hover:bg-white/90 dark:hover:bg-gray-700/80" value="favorites">
                Favs
              </TabsTrigger>
            </TabsList>
            
            <TabsContent value="overview" className="pt-2">
              <OverviewTab userName={userName} userEmail={userEmail} userJoinDate={userJoinDate} />
            </TabsContent>
