    ...
    <Card className="hover:shadow transition-shadow">
      <CardContent className="p-4">
        ...
        <Button 
          ...
          className="mt-3 p-0 text-primary font-normal"
        >
            ...
            {crawl.otherEstablishments.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-2 mt-1">
                  {crawl.otherEstablishments.map((e, i) => (
                    <Badge key={i} variant="outline" className="bg-material-surface-container-low">
                      {e}
                    </Badge>
                  ))}
                </div>
      ...
      <CardFooter className="flex justify-end gap-2 p-4 pt-0">
        {isPending && handleAcceptRequest ? (
          <Button variant="outline" onClick={() => handleAcceptRequest(crawl.id)}>
            Accept Request
          </Button>
        ) : handleEndParticipation ? (
          <Button variant="outline" onClick={() => handleEndParticipation(crawl.id)}>
            End Participation
          </Button>
        ) : null}
      </CardFooter>
