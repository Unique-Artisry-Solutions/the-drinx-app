...
const CocktailCard: React.FC<CocktailCardProps> = ({
  ...
  return <Link to={`/cocktail/${id}`}>
      <div className={cn("rounded-xl overflow-hidden elevation-2 bg-white border border-gray-100 transition-all-300 hover:elevation-3 hover:translate-y-[-2px]", className)}>
        ...
        <div className="p-4 py-[23px] bg-[#02022b]/[0.94]">
          ...
          {ingredients && ingredients.length > 0 && <div className="mt-3">
              <div className="flex flex-wrap gap-1">
                {ingredients.map((ingredient, index) => <span key={index} className="text-xs bg-material-secondary-container text-material-on-secondary-container px-2 py-1 rounded-full">
                    {ingredient}
                  </span>)}
              </div>
            </div>}
