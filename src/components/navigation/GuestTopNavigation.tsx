...
import { Home, LogIn, UserPlus, Menu, X, Sparkles, HeartHandshake, Banknote } from 'lucide-react';
      ...
      <div className="guest-nav-container max-w-6xl mx-auto px-4 py-3">
        <div className="guest-nav-inner flex items-center justify-between">
          ...
          <div className="guest-nav-right flex items-center space-x-2">
            {/* Cart button with proper stacking context */}
            ...
            <Button 
              ...
              className="guest-menu-button md:hidden text-white bg-spiritless-burgundy/20 hover:bg-spiritless-burgundy/30"
            >
            ...
            <div className="guest-auth-buttons hidden md:flex space-x-2">
              <Link to="/login">
                <Button 
                  variant="ghost" 
                  size="sm" 
                  className="guest-login-button flex items-center gap-1 text-white border-white/20 hover:bg-white/10 hover:text-white"
                >
              ...
              <Link to="/signup">
                <Button 
                  variant="default" 
                  size="sm" 
                  className="guest-signup-button flex items-center gap-1 bg-white text-spiritless-pink hover:bg-white/90"
                >
          ...
          <div className="guest-mobile-menu md:hidden py-3 space-y-2 bg-white/10 rounded-md mt-2 backdrop-blur-sm animate-fade-in">
            ...
            <div className="guest-mobile-auth flex space-x-2 px-3 py-2">
              <Link to="/login" className="flex-1">
                <Button 
                  variant="outline" 
                  className="guest-mobile-login w-full flex items-center justify-center gap-1 border-white/20 hover:bg-white/10 hover:border-white/30 text-white"
                >
                  <LogIn className="h-4 w-4" />
                  <span>Login</span>
                </Button>
              ...
              <Link to="/signup" className="flex-1">
                <Button 
                  variant="default" 
                  className="guest-mobile-signup w-full flex items-center justify-center gap-1 bg-white text-spiritless-pink hover:bg-white/90"
                >
                  <UserPlus className="h-4 w-4" />
                  <span>Sign Up</span>
                </Button>
