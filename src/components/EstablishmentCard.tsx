  ...
  return <Link to={`/establishment/${id}`} onClick={handleClick}>
      <div className={cn("flex rounded-xl overflow-hidden bg-card elevation-2 border border-border transition-all-300 hover:elevation-3 hover:translate-y-[-2px]", className)}>
        <div className="w-24 h-24 sm:w-32 sm:h-32 bg-gray-200 overflow-hidden">
          {image ? <img src={image} alt={name} className="w-full h-full object-cover transition-transform-300 hover:scale-105" /> : <div className="w-full h-full flex items-center justify-center bg-muted">
