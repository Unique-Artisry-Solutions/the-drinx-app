  ...
  const RewardsContent = () => (
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div 
          className={`text-center p-4 bg-muted/50 rounded-lg cursor-pointer hover:bg-muted/70 transition-all duration-200 hover:scale-105 ${isMobile ? 'min-h-[80px]' : ''}`}
      ...
      <div className="space-y-3">
        ...
        <AnimatedProgressBar 
          ...
          color={progressToNextTier > 80 ? 'success' : 'default'}
        />
      ...
      <div className="border-t pt-4">
        ...
        <div className="space-y-2">
          {recentRewards.map((reward, index) => (
            <div 
              key={reward.id} 
              className={`flex items-center justify-between p-2 bg-muted/30 rounded-md hover:bg-muted/50 transition-all duration-200 ${isMobile ? 'min-h-[48px]' : ''}`}
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <div className="flex items-center gap-2">
                <Gift className="h-4 w-4 text-primary animate-bounce" style={{ animationDelay: `${index * 0.2}s` }} />
