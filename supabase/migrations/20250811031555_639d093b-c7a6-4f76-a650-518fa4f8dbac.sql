-- Dev seed tracking tables and seed RPC helpers (corrected)
-- 1) Tables
...
-- Enable RLS and restrict reads to admins; writes via functions/service role
...
BEGIN
  ...
  FOR i IN 1..GREATEST(1, p_total) LOOP
    ...
    VALUES (v_review_id, v_text, v_cocktail::text, v_user, v_rating, v_source, now() - (floor(random()*30)||' days')::interval);
    created_count := created_count + 1;
    PERFORM public._log_seed_record(p_seed_run_id, 'cocktail_reviews', v_review_id);
  END LOOP;
...
-- Ensure batch grant exists (if function exists it will apply, else no-op)
COMMENT ON FUNCTION public.batch_update_user_points(jsonb) IS 'grant handled elsewhere';