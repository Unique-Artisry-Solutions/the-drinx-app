-- Phase 1: SMS Notification Database Schema Setup (Fixed)

-- First, add 'sms' to the notification_channel enum if it doesn't exist
DO $$
BEGIN
    -- Check if 'sms' exists in the enum, if not add it
    IF NOT EXISTS (
...
-- 6. Add SMS delivery status to existing notifications table (only if columns don't exist)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'notifications' AND column_name = 'sms_delivery_log_id') THEN
        ALTER TABLE public.notifications ADD COLUMN sms_delivery_log_id UUID;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'notifications' AND column_name = 'channels') THEN
        ALTER TABLE public.notifications ADD COLUMN channels JSONB DEFAULT '[]';
    END IF;
END$$;
...
-- 8. Enable RLS on all SMS tables
...
-- 9. Create RLS policies for SMS tables
...
-- 10. Create or replace update timestamp function and triggers
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_user_phone_numbers_updated_at') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_sms_delivery_logs_updated_at') THEN
